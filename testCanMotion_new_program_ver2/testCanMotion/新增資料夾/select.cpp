...
void CSocketSelectDlg::UpdateView(HANDLE hDevice)
{
  ...
  {
    SetItemText(IDC_DESCRIPTION,     "");
    SetItemText(IDC_MANUFACTURER,    "");
    SetItemText(IDC_DRIVERVERSION,   "");
    SetItemText(IDC_HARDWAREVERSION, "");
    SetItemText(IDC_DEVICECLASS,     "");
    SetItemText(IDC_HARDWAREID,      "");
    SetItemText(IDC_VCIOBJECTID,     "");
  }
}
  ...
  if (vciDeviceGetCaps(m_hCurDev, &sDevCaps) == VCI_OK)
  {
    ...
    for (UINT8 s = 0; s < sDevCaps.BusCtrlCount; s++)
    {
      if (VCI_BUS_TYPE(sDevCaps.BusCtrlTypes[s]) == m_BusType)
      {
        ...
        {
          case VCI_BUS_CAN:
            wsprintf(szText, "CAN - %c", cNumCtls + 'A');
            break;
          case VCI_BUS_LIN:
            wsprintf(szText, "LIN - %c", cNumCtls + 'A');
            break;
        }
